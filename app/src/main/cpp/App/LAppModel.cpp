...
LAppModel::LAppModel()
    : CubismUserModel(), _modelSetting(NULL), _userTimeSeconds(0.0f), _enableRandomMotion(false)
{
    if (DebugLogEnable)
    {
        _debugMode = true;
...
csmInt32 LAppModel::GetParameterCount()
{
    ...
    return this->_model->GetParameterCount();
}

Csm::csmVector<Live2D::Cubism::Framework::CubismIdHandle> LAppModel::GetParamIds()
{
    if (this->_model == NULL)
    {
        return NULL;
    ...
    return this->_model->GetParameterIds();
}

csmFloat32 *LAppModel::GetParameterValues()
{
    if (this->_model == NULL)
    {
        return NULL;
    ...
    return this->_model->GetParameterValues();
}

const csmFloat32 *LAppModel::GetParameterMaximumValues()
{
    if (this->_model == NULL)
    {
        return NULL;
    ...
    return this->_model->GetParameterMaximumValues();
}

const csmFloat32 *LAppModel::GetParameterMinimumValues()
{
    if (this->_model == NULL)
    {
        return NULL;
...
const csmFloat32 *LAppModel::GetParameterDefaultValues()
{
    if (this->_model == NULL)
    {
        return NULL;
    ...
    return this->_model->GetParameterDefaultValues();
}

void LAppModel::SetParamValue(Csm::csmString id, csmFloat32 value)
{
    if (this->_model == NULL)
    {
        return;
    ...
    const csmInt32 index = this->_model->GetParameterIndex(CubismFramework::GetIdManager()->GetId(id));

    this->_model->AddParameterValue(index, value);
}
...
void LAppModel::SetupModel(ICubismModelSetting *setting)
{
    ...
    // Breath
    {
        ...
        if (_idParamBraeth == NULL)
        {
            _idParamBraeth = CubismFramework::GetIdManager()->GetId(ParamBreath);
        }
        breathParameters.PushBack(CubismBreath::BreathParameterData(_idParamBraeth, 0.5f, 0.5f, 3.2345f, 0.5f));
...
void LAppModel::Update()
{
    ...
    if (_motionManager->IsFinished() && _enableRandomMotion)
    {
...
int LAppModel::GetExpressionSize()
{
    return _expressions.GetSize();
}

int LAppModel::GetMotionSize()
{
    return _motions.GetSize();
}

Csm::csmString LAppModel::GetExpression(int index)
{
...
Csm::csmString LAppModel::GetMotion(int index)
{
    csmInt32 no = rand() % _motions.GetSize();
    csmMap<csmString, ACubismMotion *>::const_iterator map_ite;
    int i = 0;
    for (map_ite = _motions.Begin(); map_ite != _motions.End(); map_ite++)
    {
        if (i == index)
        {
            return (*map_ite).First;
